...
const Login: React.FC = () => {
    ...
    return (
        <div>
           {widget == 'login' && <LoginCard setWidget={setWidget} />}
           {widget == 'signup' && <Signup setWidget={setWidget} />}
           {widget == 'forgetPassword' && <ForgotPasswordCard setWidget={setWidget} />}
        </div>
    );
};
...
const LoginCard: React.FC<AuthCardProps> = ({ setWidget }) => {
        ...
        <div className="flex justify-center items-center min-h-screen">
            <Card className="text-center">
                ...
                <div className="mt-4">
                    <p
                        className="text-blue-600 hover:underline text-sm cursor-pointer"
                        onClick={() => setWidget('forgetPassword')}
                    >
                        Forgot Password?
                    </p>
                </div>
...
const Signup: React.FC<AuthCardProps> = ({ setWidget }) => {
    const [loading, setLoading] = useState(false);
    const [showPassword, setShowPassword] = useState(false);
    ...
    const onSubmit: SubmitHandler<SignupFormInputs> = async (data) => {
        setLoading(true);
        toast.promise(
            axios.post('https://x8ki-letl-twmt.n7.xano.io/api:IAuUhB5G/auth/signup', { ...data }),
            {
                loading: 'Signing Up...',
                success: () => {
                    setWidget('login');
                        ...
                        <div className="relative">
                            <input
                                ...
                                {...register("password", {
                                    ...
                                    pattern: {
                                        value: /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/,
                                        message: "* Password must be alphanumeric and include at least one uppercase letter, one lowercase letter, one number, and one special character",
                                    },
                                })}
...
const ForgotPasswordCard: React.FC<AuthCardProps> = ({ setWidget }) => {
